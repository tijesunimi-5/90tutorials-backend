...
/**
 * 2. Student Attempts Route (Updated with Subject Join)
 */
router.get("/student/attempts", validateSession, async (request, response) => {
  ...
  try {
    ...
    // 🟢 FETCH DETAILS WITH SUBJECT NAME
    const detailRows = (
      await client.query(
        `SELECT aa.*, q.question_text, s.name AS subject_name, 
...
/**
 * 3. Admin Summary Route (Updated with Subject Join)
 */
router.get(
  ...
  async (request, response) => {
    ...
    try {
      const summaryRows = (
        await client.query(
          `SELECT ea.*, u.name AS student_name, 
           ...
           WHERE ea.exam_id = $1 ORDER BY ea.total_score DESC, ea.time_taken_seconds ASC`,
          [examId],
        )
      ).rows;
      ...
      const breakdownsByAttempt = subjectBreakdowns.reduce((acc, row) => {
        acc[row.attempt_id] = acc[row.attempt_id] || [];
        acc[row.attempt_id].push(row);
        return acc;
      }, {});
      ...
      const finalData = summaryRows.map((s) => ({
        ...
        subject_breakdown: breakdownsByAttempt[s.attempt_id] || [],
      }));
...
/**
 * Remaining routes (check status, get exams) remain the same...
 */
