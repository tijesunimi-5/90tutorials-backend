...
router.post("/results/submit", validateSession, async (request, response) => {
  ...
  try {
    ...
    // 5. Construct ID Code (Prefix/YY/Num)
    ...
    // 6. Return response (Hide score if not released)
    return response.status(201).send({
      message: isReleased
        ? "Exam results saved."
        : "Exam submitted. Results are pending release.",
      data: {
        attemptId,
        finalScore: isReleased ? finalScore : null,
        studentIdCode,
...
router.get("/student/attempts", validateSession, async (request, response) => {
  ...
  try {
    ...
    // Fetch attempts with release date info
    const summaryQuery = `
    ...
    if (attemptIds.length > 0) {
      const detailRows = await client.query(
        `SELECT aa.*, q.question_text FROM attempt_answers aa JOIN questions q ON aa.question_id = q.question_id WHERE aa.attempt_id = ANY($1::int[])`,
        [attemptIds]
      );
      detailsMap = detailRows.rows.reduce((acc, row) => {
    ...
    const finalData = summaryRows.map((summary) => {
      const isReleased =
        !summary.results_release_at ||
        new Date(summary.results_release_at) <= new Date();
      ...
      return {
        ...
        // Only return these if released
        total_score: isReleased ? parseFloat(summary.total_score) : null,
        correct_answers: isReleased ? summary.correct_answers : null,
        total_questions: summary.total_questions,
        details: isReleased ? detailsMap[summary.attempt_id] || [] : [],
        is_released: isReleased,
        release_date: summary.results_release_at,
      };
    });
    ...
    return response
      .status(500)
      .send({ message: "Fetch failed.", error: error.message });
  } finally {
    client.release();
  }
});
